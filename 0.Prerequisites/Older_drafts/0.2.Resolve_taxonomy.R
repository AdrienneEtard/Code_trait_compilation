...
# (4) For species that does not appear in the Red List, extract synonyms from ITIS
...
source("0.2.Resolve_taxonomy_functions.R")
...
Meiri_0 <- read.csv("../../Data/Reptiles/Meiri_0ReptileMasses.csv", sep=",")
...
## 1.2. Create lists of species names : check that all have two words (binomial), and return original species list
...
#        list(PredictsMammals, PhyloMammal, MyhrvoldMammal, Pacifici, Kissling, Pantheria, Mammal_range, IUCN_mammal))
# 
# Birds <- Reduce(function(x,y) merge(x = x, y = y, by = "Original", all=TRUE),
#                   list(PredictsBirds, PhyloBird, MyhrvoldBird, Butchart_BM, Butchart_GL, Sekercioglu_Diet, Bird_range, IUCN_bird))
# 
# Reptiles <- Reduce(function(x,y) merge(x = x, y = y, by = "Original", all=TRUE),
#                    list(PredictsReptiles, PhyloReptile, MyhrvoldReptile, Meiri_0, Scharf, Reptile_range, IUCN_reptile))
# 
...
# ListBirds <- subset(ListBirds, Original!="'Scytalopus sp.") # this species is in the phylogeny
# 
# 
# ## Save these files
# write.csv(ListBirds, "../../Results/0.Data_resolved_taxonomy/ListBirds.csv", row.names = FALSE)
# write.csv(ListMammals, "../../Results/0.Data_resolved_taxonomy/ListMammals.csv", row.names = FALSE)
# write.csv(ListAmphibians, "../../Results/0.Data_resolved_taxonomy/ListAmphibians.csv", row.names = FALSE)
# write.csv(ListReptiles, "../../Results/0.Data_resolved_taxonomy/ListReptiles.csv", row.names = FALSE)
# 
...
## Load saved files
# Mammals <- read.csv("../../Results/0.Data_resolved_taxonomy/List_before_resolving/ListMammals.csv")
# Amphibians <- read.csv("../../Results/0.Data_resolved_taxonomy/List_before_resolving/ListAmphibians.csv")
# Birds <- read.csv("../../Results/0.Data_resolved_taxonomy/List_before_resolving/ListBirds.csv")
# Reptiles <- read.csv("../../Results/0.Data_resolved_taxonomy/List_before_resolving/ListReptiles.csv")
...
Reptiles <- read.csv("../../Results/0.Data_resolved_taxonomy/List_before_resolving/ListReptiles.csv")
# 
## Delta species with Elton traits
Elton_birds_supp <- SpNames(Elton_birds$Scientific) %>% 
  filter(Original!="")
Elton_birds_supp  <- Elton_birds_supp %>% CheckTypos()

Elton_birds_supp <- setdiff(Elton_birds$Scientific, Birds$CorrectedTypos) %>%
  as.data.frame() %>%
  setNames(., "Original") %>%
  filter(Original != "") %>%
  mutate(Original = as.character(Original)) 

Elton_mammals_supp <- SpNames(Elton_mammals$Scientific) %>% 
  filter(Original!="")
Elton_mammals_supp  <- Elton_mammals_supp %>% CheckTypos()

Elton_mammals_supp <- setdiff(Elton_mammals$Scientific, Mammals$CorrectedTypos)%>%
  as.data.frame() %>%
  setNames(., "Original") %>%
  filter(Original != "") %>%
  mutate(Original = as.character(Original))
...
Vidan_sp <- SpNames(Vidan$Species) %>% CheckTypos()
Stark_sp <- SpNames(Stark$species)  %>% 
  filter(Original!="") %>%
  CheckTypos()
Schwarz_sp <- SpNames(Schwarz$Species) %>% 
  filter(Original!="") %>%
  CheckTypos()
Nov1_sp <- SpNames(Novosolov$Binomial) %>% 
  filter(Original!="") %>%
  CheckTypos()
Nov2_sp <- SpNames(Novosolov_2$species) %>% 
  filter(Original!="") %>%
  CheckTypos()
Slavenko_sp <- SpNames(Slavenko$Binomial) %>% 
  filter(Original!="") %>%
  CheckTypos()
Meiri_sp <- SpNames(Meiri$species) %>% 
  filter(Original!="") %>%
  CheckTypos()

ToAddRep <- function(DF, SynRep) {
  Sp <- DF$CorrectedTypos
  Delta <- intersect(Sp, SynRep$CorrectedTypos)
  Delta <- setdiff(Sp, Delta)
  ToReturn <- subset(DF, CorrectedTypos %in% Delta)
  return(ToReturn)
}

To_add <- rbind(ToAddRep(Vidan_sp, Reptiles), ToAddRep(Stark_sp, Reptiles), ToAddRep(Schwarz_sp, Reptiles), ToAddRep(Nov1_sp, Reptiles),
                ToAddRep(Nov2_sp, Reptiles), ToAddRep(Slavenko_sp, Reptiles), ToAddRep(Meiri_sp, Reptiles))
To_add <- unique(To_add[c("Original", "CorrectedTypos", "IsCorrected")])

## Mammals

# Mammals$CorrectedTypos <- as.character(Mammals$CorrectedTypos)
# Synonym_mammals <- RunSyn(Mammals)
# write.csv(Synonym_mammals, "../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_mammals.csv", row.names=F)
 
## Amphibians

# Amphibians$CorrectedTypos <- as.character(Amphibians$CorrectedTypos)
# Synonym_amphibians <- RunSyn(Amphibians)
# write.csv(Synonym_amphibians, "../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_amphibians.csv", row.names=F)
...
# write.csv(Synonym_reptiles,"../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_reptiles.csv", row.names = FALSE)

## Birds

# Birds$CorrectedTypos <- as.character(Birds$CorrectedTypos)
# Birds <- RunSynSave(Birds)
# 
...
# write.csv(Synonym_birds,"../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_birds.csv", row.names = FALSE)

# ## Elton supp
# Elton_birds_supp$CorrectedTypos <- as.character(Elton_birds_supp$CorrectedTypos)
# Elton_birds_toadd <- Redlist_synonyms(Elton_birds_supp)
# 
# Elton_mammals_supp$CorrectedTypos <- as.character(Elton_mammals_supp$CorrectedTypos)
# Elton_mammals_toadd <- Redlist_synonyms(Elton_mammals_supp)
# 


## Reptiles supp
To_add$CorrectedTypos <- as.character(To_add$CorrectedTypos)
ToAddReptiles <- Redlist_synonyms(To_add)
write.csv(ToAddReptiles, "../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Reptiles_toAdd.csv", row.names=FALSE)


ToAddReptiles <- read.csv("../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Reptiles_toAdd.csv")
RetrieveReptiles <- subset(ToAddReptiles, CorrectedTypos!="Sitana spinaecephalus")
RetrieveReptiles <- Complement_ITIS(RetrieveReptiles)


Syn_Reptiles <- read.csv("../../Results/0.Data_resolved_taxonomy/List_species_synonyms/After_manual_checks/Reptiles.csv")
Columns <- setdiff(colnames(Syn_Reptiles), colnames(ToAddReptiles))
ToAddReptiles$InITIS <- NA
ToAddReptiles$Genus_level <- NA
ToAddReptiles$Genus <- word(ToAddReptiles$Accepted,1)
ToAddReptiles$CorrectedTypos <- as.character(ToAddReptiles$CorrectedTypos)
ToAddReptiles$Accepted <- as.character(ToAddReptiles$Accepted)
ToAddReptiles$Accepted[is.na(ToAddReptiles$Accepted)]<- ToAddReptiles$CorrectedTypos[is.na(ToAddReptiles$Accepted)]

SynReptiles <- rbind(Syn_Reptiles, ToAddReptiles)
SynReptiles <- SynReptiles[order(SynReptiles$Original),]
write.csv(SynReptiles, "../../Results/0.Data_resolved_taxonomy/List_species_synonyms/After_manual_checks/Reptiles_V2_added.csv", row.names = FALSE)


# Sitana spinaecephalus is problematic
                            # #  3. Extracting synonyms - from ITIS  # # 


# Load data (list of synonyms)
# Syn_Mammals <- read.csv("../../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_mammals.csv")
# Syn_Amphibians <- read.csv("../../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_amphibians.csv")
# Syn_Birds <- read.csv("../../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_birds.csv")
# Syn_Reptiles <- read.csv("../../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_reptiles.csv")

# Complement the synonyms datasets with those from ITIS
...
# Syn_Mammals <- Complement_ITIS(Syn_Mammals)
...
# Syn_Reptiles <- Complement_ITIS(Syn_Reptiles)

# Syn_Birds <- Complement_ITIS(Syn_Birds)


# Save file after addition of ITIS synonyms
# write.csv(Syn_Mammals, "../../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_mammals_ITIS.csv", row.names = FALSE)
# write.csv(Syn_Amphibians, "../../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_amphibians_ITIS.csv", row.names = FALSE)
# write.csv(Syn_Reptiles, "../../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_reptiles_ITIS.csv", row.names = FALSE)
# write.csv(Syn_Birds, "../../../Results/0.Data_resolved_taxonomy/List_species_synonyms/Before_manual_checks/Synonyms_birds_ITIS.csv", row.names = FALSE)
# 
# 
# Elton_birds_toadd <- Complement_ITIS(Elton_birds_toadd)
# Elton_birds_toadd$Accepted[Elton_birds_toadd$CorrectedTypos=="Parus teneriffae"] <- "Cyanistes teneriffae"
# Elton_birds_toadd$Accepted[Elton_birds_toadd$CorrectedTypos=="Nectarinia tsavoensis"] <- "Cinnyris tsavoensis"
# Elton_birds_toadd$Accepted[2] <- "Setophaga aestiva"
# Elton_birds_toadd$Accepted[Elton_birds_toadd$CorrectedTypos=="Basileuterus chlorophrys"] <- "Myiothlypis chlorophrys"
# Elton_birds_toadd$Accepted[Elton_birds_toadd$CorrectedTypos=="Parus carpi"] <- "Melaniparus carpi"
# 
# Elton_birds_toadd$Order <- toupper(c(rep("Passeriformes",9)))
# Elton_birds_toadd$Family  <- toupper(c("Parulidae", "Parulidae", rep("Nectariniidae",3),"Estrildidae",rep("Paridae",2), "Corvidae"))
# 
# Elton_mammals_toadd <- Complement_ITIS(Elton_mammals_toadd)
# Elton_mammals_toadd$CorrectedTypos[Elton_mammals_toadd$Original=="Leimacomys buttneri"] <- "Leimacomys buettneri"
# Elton_mammals_toadd$Accepted[Elton_mammals_toadd$Original=="Leimacomys buttneri"] <- "Leimacomys buettneri"
# Elton_mammals_toadd$Order[Elton_mammals_toadd$Original=="Leimacomys buttneri"] <- toupper("Rodentia")
# Elton_mammals_toadd$Family[Elton_mammals_toadd$Original=="Leimacomys buttneri"] <- toupper("Muridae")
# Elton_mammals_toadd$Order[Elton_mammals_toadd$Original=="Spilogale aquaticus"] <- toupper("Carnivora")
# Elton_mammals_toadd$Family[Elton_mammals_toadd$Original=="Spilogale aquaticus"] <- toupper("Mephitidae")
# Elton_mammals_toadd$Accepted[Elton_mammals_toadd$Original=="Spilogale aquaticus"] <- "Spilogale aquaticus"
...
# Syn_Reptiles <- read.csv("../../Results/0.Data_resolved_taxonomy/List_species_synonyms/After_manual_checks/Reptiles.csv")
Syn_Reptiles <- read.csv("../../Results/0.Data_resolved_taxonomy/List_species_synonyms/After_manual_checks/Reptiles_V2_added.csv")
Syn_Amphibians <- read.csv("../../Results/0.Data_resolved_taxonomy/List_species_synonyms/After_manual_checks/Amphibians.csv")
...
# Slavenko$Binomial %<>% as.character()
# Slavenko$Binomial[Slavenko$Binomial=="Spondylurus anegadae"] <- "Spondylurus anegadeae"
