...
CheckTypos <- function(SpDF) {
  ...
  SpDF <- SpDF %>% select(Original, CorrectedTypos)
   ...
   FuzzyMatch <- FALSE
    ...
    if (X$count==0) {
      ...
      id <- Search$taxonid
        ...
        # 1.1. If id is null: no record in the Red List -- manual check will be required.
    ...
    ## 2. Synonyms are found
      ...
      InRedList <- TRUE
        ...
        # with any because in some cases, names differ (with Acomys cahirinus for example)
        ...
        # 2.2 If the name is a synonym  
          ...
          ## Special cases -- when there are several accepted names: Find the best match between the name and the accepted names
          ...
          if (Y$count==0) {
              ...
              # if there are more than one accepted names appearing because of subspecies: just choose ToLook as the accepted name
