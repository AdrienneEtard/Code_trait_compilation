...
X <- AE_Imputed$M$Imputed.Dataset %>%
  select(Best_guess_binomial, Diel_activity, log10_Body_mass_g, log10_Litter_size, sqrt_Habitat_breadth_IUCN,Phylo_info) %>%
  mutate(Class="Mammalia")
...
p1 <- Compare(RC_Collected, NULL, AE_Collected, NULL, "body_mass_median","log10_Body_mass_g", FALSE,"BM","collected", "collected", FALSE, FALSE, FALSE)  + labs(tag = "B") + theme(plot.tag.position = c(0.17, 0.94))
p2 <- Compare(RC_Collected, NULL, AE_Collected, NULL,  "litter_clutch_size","log10_Litter_size", FALSE,"LCS", "collected", "collected",FALSE,FALSE, FALSE)+ labs(tag = "A") + theme(plot.tag.position = c(0.17, 0.94))
p3 <- Compare(RC_Collected, NULL, AE_Collected, NULL,"hab_breadth","sqrt_Habitat_breadth_IUCN", FALSE, NULL, "collected", "collected", FALSE,FALSE, FALSE)+ labs(tag = "C") + theme(plot.tag.position = c(0.17, 0.94))
p4 <- Compare(RC_Collected, NULL,  AE_Collected, NULL,  "activity","Diel_activity", TRUE, "DA", "collected", "collected", FALSE,FALSE, FALSE)+ labs(tag = "D") + theme(plot.tag.position = c(0.17, 0.94))
p <- ggarrange(p2,p1,p3,p4,common.legend = TRUE, legend="top")
...
p2 <- Compare(RC_Collected, RC_Imputed, AE_Collected, AE_Imputed,  "litter_clutch_size","log10_Litter_size", FALSE,"LCS", "imputed", "imputed",TRUE,FALSE, FALSE)+ labs(tag = "A") + theme(plot.tag.position = c(0.17, 0.94))
p3 <- Compare(RC_Collected, RC_Imputed, AE_Collected, AE_Imputed,"hab_breadth","sqrt_Habitat_breadth_IUCN", FALSE, NULL, "imputed", "imputed", TRUE, FALSE, FALSE)+ labs(tag = "C") + theme(plot.tag.position = c(0.17, 0.94))
p4 <- Compare(RC_Collected, RC_Imputed,  AE_Collected, AE_Imputed,  "activity","Diel_activity", TRUE, "DA", "imputed","imputed", TRUE, FALSE, FALSE)+ labs(tag = "B") + theme(plot.tag.position = c(0.17, 0.94))
p <- ggarrange(p2,p4,p3,common.legend = TRUE)
...
p <- ggarrange(p2,p3,p2b,p3b,p1,p4,common.legend = TRUE, ncol=2, nrow=3)
