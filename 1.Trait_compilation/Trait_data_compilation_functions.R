...
.Normalise_TDB <- function(Kissling, Elton_MD, Pantheria, Pacifici, Myhrvold, 
  ...
  # Cooper
  Amphibian_traits.Cooper <- c("Habitat_breadth", "Svl_length_mm", "Range_size_m2", "Litter_size")
  colnames(Cooper)[c( 4, 7, 8, 9)] <- Amphibian_traits.Cooper
  ...
  # Bickford
  Amphibian_traits.Bickford <- c("Range_size_m2", "Body_length_mm", "Terrestriality")
  colnames(Bickford)[c(5, 9, 10)] <- Amphibian_traits.Bickford
  Bickford$Range_size_m2 <- as.numeric(Bickford$Range_size_m2) * 1000000
  
  # Scharf reptile
  Reptile_traits.Scharf <- c("Longevity_d", "Body_mass_g", "Diel_activity", "Trophic_level", "Litter_size", "Maturity_d")
  ...
  Scharf$Longevity_d <- Scharf$Longevity_d * 365.25
  ...
  Traits.Novosolov <- c("Range_size_m2", "Body_mass_g", "Trophic_level")
  colnames(Novosolov)[c(2,4,16)] <- Traits.Novosolov
...
.MutHabIUCN <- function(CatTrait) {
  ...
  CatTrait  %<>% group_by(Best_guess_binomial)%>%
    ...
    mutate(Marine.intertidal.or.coastal.supratidal=
